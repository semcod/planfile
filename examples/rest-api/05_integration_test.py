...
class TestPlanfileAPI:
    ...
    def test_update_ticket(self, client):
        ...
        # Create
        create_resp = client.post("/tickets", json={"title": "Update Test", "priority": "low"})
        ticket = create_resp.json()

        # Update
        update_resp = client.patch(
            f"/tickets/{ticket['id']}", json={"status": "in_progress", "priority": "critical"}
        )
    ...
    def test_move_ticket(self, client):
        ...
        # Create
        create_resp = client.post("/tickets", json={"title": "Move Test", "sprint": "source"})
        ticket = create_resp.json()

        # Move
        move_resp = client.post(f"/tickets/{ticket['id']}/move?to_sprint=destination")
...
def run_tests():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pytest", __file__, "-v"], cwd=str(Path(__file__).parent)
    )
