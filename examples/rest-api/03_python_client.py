...
class PlanfileClient:
    ...
    def list_tickets(
        self, sprint: str | None = None, status: str | None = None, priority: str | None = None
    ) -> list:
    ...
    def move_ticket(self, ticket_id: str, to_sprint: str) -> dict:
        ...
        return self._request("POST", f"/tickets/{ticket_id}/move", params={"to_sprint": to_sprint})
...
def example_basic_operations(client):
    ...
    # Update
    updated = client.update_ticket(ticket["id"], status="in_progress", assignee="developer")
...
def example_bulk_operations(client):
    ...
    for i in range(3):
        ticket = client.create_ticket(
            title=f"Bulk Ticket {i + 1}: API Test", priority="medium", sprint="current"
        )
