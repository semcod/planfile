...
![version](https://img.shields.io/badge/version-0.1.0-blue) ![python](https://img.shields.io/badge/python-%3E%3D3.10-blue) ![coverage](https://img.shields.io/badge/coverage-unknown-lightgrey) ![functions](https://img.shields.io/badge/functions-607-green)
> **607** functions | **70** classes | **173** files | CC̄ = 3.9
...
```
planfile/
├── cleanup_redundant├── auto_generate_planfile├── docker-entrypoint├── run_examples├── project    ├── validate_with_llx    ├── run        ├── run        ├── run        ├── 04_javascript_client        ├── 01_start_server        ├── run_all        ├── 02_curl_examples├── mcp-server-example        ├── run        ├── run        ├── run        ├── run        ├── run        ├── run    ├── llx_validator        ├── run        ├── run        ├── 03_python_client        ├── run        ├── ci-workflow        ├── demo    ├── PROPOSED_API_IMPROVEMENTS        ├── 01_full_workflow        ├── 03_proxy_routing        ├── run_all        ├── 02_mcp_integration        ├── 04_analytics_simple        ├── 04_advanced_filtering        ├── 03_integration_simple        ├── 02_ticket_management        ├── run        ├── 01_basic_usage        ├── planfile-sync        ├── run        ├── run_fixed        ├── run        ├── run        ├── run_fixed        ├── verify_planfile        ├── run    ├── server_common        ├── 03_integration    ├── examples    ├── execution├── planfile/    ├── runner    ├── models        ├── 04_llx_integration        ├── state    ├── executor_standalone    ├── sync/    ├── ci        ├── mock        ├── utils        ├── github        ├── operations        ├── generic        ├── markdown_backend/            ├── files            ├── tickets            ├── backend            ├── constants        ├── jira        ├── yaml_loader    ├── loaders/        ├── cli_loader        ├── external_tools    ├── analysis/        ├── gitlab        ├── base        ├── generator        ├── generators/        ├── models        ├── sprint_generator            ├── metrics_extractor        ├── parsers/            ├── json_parser        ├── file_analyzer            ├── yaml_parser    ├── core/        ├── store            ├── text_parser        ├── models/        ├── store_files        ├── store_tickets            ├── ticket            ├── toon_parser            ├── base        ├── common    ├── importers/        ├── yaml_importer        ├── json_importer        ├── redup_importer        ├── auto_loop        ├── commands        ├── extra_commands        ├── project_detector/    ├── cli/        ├── __main__                ├── commands            ├── sync/            ├── strategy                ├── commands                ├── core            ├── review/        ├── code2llm_importer            ├── health/                ├── commands                ├── utils            ├── examples/            ├── query/                ├── commands            ├── apply/                ├── commands                ├── commands                ├── utils            ├── init/            ├── auto/                ├── commands                ├── commands            ├── generate/            ├── ticket/                ├── commands            ├── validate/            ├── progress                ├── commands            ├── errors        ├── core/            ├── registry            ├── console            ├── fallback            ├── readme            ├── license            ├── package            ├── base            ├── structure                ├── commands            ├── git            ├── model_tier            ├── main            ├── gates            ├── pyproject            ├── inference        ├── adapters    ├── llm/        ├── prompts        ├── generator        ├── client    ├── utils/        ├── priorities    ├── extensions/    ├── mcp/        ├── metrics        ├── vallm_importer    ├── integrations/        ├── server        ├── base        ├── gitlab        ├── github        ├── generic    ├── api/        ├── server        ├── jira        ├── config```
