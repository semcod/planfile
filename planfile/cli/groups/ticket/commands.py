...
def ticket_update(ticket_id: str=typer.Argument(..., help='Ticket ID'), status: str | None=typer.Option(None, help='New status'), priority: str | None=typer.Option(None, '-p', '--priority'), title: str | None=typer.Option(None, help='New title')) -> None:
...
def ticket_review(ticket_id: str=typer.Argument(..., help='Ticket ID to send for review')) -> None:
...
def ticket_export_todo(todo_file: str=typer.Option('TODO.md', '--file', help='TODO.md file path'), sprint: str=typer.Option('all', '-s', '--sprint', help='Sprint to export (current/backlog/all)'), include_done: bool=typer.Option(True, '--include-done/--skip-done')) -> None:
    ...
    lines = ['# TODO', '', '<!-- AUTO-GENERATED FROM PLANFILE - DO NOT EDIT DIRECTLY -->', '<!-- Use: planfile ticket export-todo to regenerate -->', '<!-- Use: planfile ticket import-todo to import changes -->', f'<!-- Generated: {datetime.now().isoformat()} -->', '']
    priority_emoji = {'critical': '🔴', 'high': '🟠', 'medium': '🟡', 'low': '🟢', 'normal': '⚪'}
