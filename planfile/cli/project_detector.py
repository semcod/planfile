...
def _find_readme_content(project_path: Path) -> tuple[Optional[str], Optional[str]]:
    """
...
def _find_readme_goal(project_path: Path) -> Optional[str]:
    """Extract goal/summary from README."""
    _, goal = _find_readme_content(project_path)
    return goal


def _detect_quality_gates(project_path: Path, pyproject_data: Optional[dict] = None) -> list[DetectedQualityGate]:
...
def _detect_license(project_path: Path) -> Optional[str]:
...
def _detect_model_tier(project_path: Path) -> Optional[str]:
