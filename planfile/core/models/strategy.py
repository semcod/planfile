            ...
            for item in v:
                if isinstance(item, dict):
                    if 'task_patterns' in item:
                        continue
                    elif 'name' in item or 'title' in item:
...
class Strategy(BaseModel):
    ...
    def compare(self, other: 'Strategy') -> dict[str, Any]:
        ...
        comparison = {'common_elements': [], 'differences': [], 'only_in_self': [], 'only_in_other': [], 'similarity_score': 0.0}
        if self.name == other.name:
            comparison['common_elements'].append(f'Name: {self.name}')
        else:
            comparison['differences'].append({'field': 'name', 'self': self.name, 'other': other.name})
        ...
        if self_goal == other_goal:
            ...
            comparison['differences'].append({'field': 'goal', 'self': self_goal, 'other': other_goal})
        self_sprint_ids = {s.id for s in self.sprints}
        other_sprint_ids = {s.id for s in other.sprints}
        comparison['common_elements'].extend([f'Sprint {sid}' for sid in self_sprint_ids & other_sprint_ids])
        comparison['only_in_self'].extend([f'Sprint {sid}' for sid in self_sprint_ids - other_sprint_ids])
        comparison['only_in_other'].extend([f'Sprint {sid}' for sid in other_sprint_ids - self_sprint_ids])
